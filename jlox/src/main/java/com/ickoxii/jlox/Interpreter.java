...
class Interpreter implements Expr.Visitor<Object>,
   ...
   *
   * We also store a reference to the superclass (if it exists)
   * and create LoxFunctions for each method. Those capture the
   * current environment - the one where we just bound "super"
   * as their closure, which holds on to the superclass.
   * */
  @Override
  ...
  /**
   * Looks up the surrounding class's superclass by looking
   * up "super" in the proper environment.
   *
   * When accessing a method, we need to bind `this` to the
   * object the method is accessed from. Even though we are
   * looking up the method on the superclass, the _instance_
   * is still `this`.
   * */
  @Override
  public Object visitSuperExpr(Expr.Super expr) {
    ...
    if (method == null) {
      throw new RuntimeError(expr.method,
          "Undefine property '" + expr.method.lexeme + "'.");
    }
  ...
  public Object visitBinaryExpr(Expr.Binary expr) {
    ...
    switch (expr.operator.type) {
      ...
      case PLUS:
        ...
        /** Allow for concatenation of strings */
